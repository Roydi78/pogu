...
const PILLARS = [
  ...
  { icon: '🧊', label: 'Toujours frais' },
  { icon: '🚀', label: 'Livré chez toi' },
  { icon: '🎨', label: 'Design unique' },
]
...
export default function Concept() {
  ...
  return (
    <section id="concept" className="py-32 bg-pogu-black" ref={ref}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          >
            ...
            <div className="absolute -bottom-5 -right-5 bg-pogu-yellow text-pogu-black font-head font-black text-xl text-center leading-tight px-6 py-5 rounded-2xl shadow-[0_12px_40px_rgba(245,197,24,0.3)]">
          ...
          >
            ...
            <h2 className="font-head text-[clamp(2rem,4.5vw,3.5rem)] font-black leading-[1.1] tracking-tight text-white mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {PILLARS.map((p) => (
                <div
                  key={p.label}
                  className="flex items-center gap-3 bg-pogu-surface border border-white/[0.08] rounded-2xl px-4 py-4 hover:border-pogu-yellow/30 hover:bg-pogu-surface2 transition-all"
                >
                  <span className="text-2xl">{p.icon}</span>
                  <span className="font-head text-sm font-semibold text-white">{p.label}</span>
                </div>
              ))}
            </div>
          </div>
